...
serve(async (req) => {
  ...
  try {
    ...
    if (data?.trim()) userContent.push({ type: "text", text: `Clean and structure this data into a table:\n\n${data}` });
    if (image) {
      if (!data?.trim()) userContent.push({ type: "text", text: "Extract and structure the data from this image into a table:" });
    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `You are a data structuring AI. The user will paste messy, unstructured data or provide an image containing data. You must:
